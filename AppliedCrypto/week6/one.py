...
if __name__ == "__main__":
    ...
    #B
    Bresponse = oracle.encrypt(user_input*answer*4) #Encrypt "A" (16*4)A's
    ...
    front_pad = None
    for i in range(0,65):
        Bpresponse = oracle.encrypt(user_input*i)
        detected = rypto.ecb_detect(Bpresponse)
        if detected:
            front_pad = i-32
            print len(oracle.random)
            print "Front padding requires " + str(front_pad) + " more bytes"
            front_pad = "X"*front_pad
            print front_pad
            break;
        else:
            pass
    ...
    for h in range(1,50):
        ...
        """
        This creates two blocks and uses for loop to remove a's from blocks
        """
        for i in range(1,answer+1):
            cblock1 = front_pad + "A" * (answer-i) + full_string + block #First block grows each time
            ...
            for j in range(0,256):
                ...
                #print try_string,len(oracle.random)
                hx_try = try_string.encode("hex") #Convert to hex to encrypt (required)
                encrypted = oracle.encrypt(hx_try) #Returns ascii

                print try_string
                #print rypto.split(encrypted.encode("hex"),32)
                """
